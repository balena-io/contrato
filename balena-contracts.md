...
| provides | [ [ContractRange](#contractrange) ] | Allows to specify what functionalities or capabilities an entity defined by the contract provides.  Differently from requirements, only a list of contract references is supported for now  Example: ```yml type: sw.container slug: balena-browser requires:   - type: sw.application     slug: "balena-supervisor"     version: ">=10.6.7" ``` | No |
...
A reference to a contract or a range of contracts
If version is not defined, any contract of the same
type and slug is accepted
