...
//! type+slug.
//!
//! Holds all of a contract's direct children in an insertion-ordered
//! [`IndexMap`] keyed by hash, together with secondary indexes that
//! support the public
...
impl ContractIndex {
    ...
    pub(crate) fn remove(&mut self, contract: &Contract) -> Mutation {
        ...
        self.map.shift_remove(&child_hash);

        if let Some(hashes) = self.by_type.get_mut(&ty) {
            hashes.shift_remove(&child_hash);
            if hashes.is_empty() {
